...
function visitorColor(presence: { playerIdentity?: { playerStyle?: { colorPalette?: string[] } } }) {
  return presence.playerIdentity?.playerStyle?.colorPalette?.[0] ?? '#d9cab3'
}

function visitorLabel(presence: { playerIdentity?: { name?: string; publicKey?: string } }, index: number) {
...
export function LiveSignals() {
  ...
  const people = Array.from(presences.values())
  const others = people.filter((presence) => presence.playerIdentity?.publicKey !== myPublicKey)
  const total = people.length || (isLoading ? 0 : 1)
  ...
  return (
    ...
    >
      ...
      <span className="live-signals__dots" aria-hidden="true">
        {people.slice(0, 5).map((presence, index) => (
          <span
      ...
      {others.length > 0 ? <span className="live-signals__hint">same page</span> : null}
    </aside>
  )
}
